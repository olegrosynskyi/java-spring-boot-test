...
public class AdGroupTable extends TableImpl<Record> {

    public final static AdGroupTable TABLE = new AdGroupTable(DSL.name("ad_groups"));

    public AdGroupTable(Name name) {
        super(name);
    }

    public final TableField<Record, Long> id = createField(DSL.name("id"), SQLDataType.BIGINT);
    public final TableField<Record, Long> campaignId = createField(DSL.name("campaign_id"), SQLDataType.BIGINT);
